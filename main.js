...
$(function () {
  ...
  var fromButtonSelect, fromGeojsonUpdate;
  ...
  // const osdLayer = document.createElement('div');
  // osdLayer.id = 'osd';
  // mapElem.append(osdLayer);

  const height = 9221;
  const width = 7026;
  const tsize = 256;

  var params = geo.util.pixelCoordinateParams('#osdViewer', width, height, tsize, tsize);
  params.map.clampZoom = false;
  params.map.clampBoundsX = false;
  params.map.clampBoundsY = false;

  const mapParams = Object.assign({}, params.map, {node: '#map'});
  ...
  function change_controls(evt) {
    ...
    if (param === 'editmode' && value === 'true') {
      $('#map').addClass('active-edit');
    } else {
      $('#map').removeClass('active-edit');
    }
...
/*function for getting variables from url*/
...
function getHelpersImg(){
  return getUrlParameter('imageDzi');
}

function postData(token,data){
  console.log(JSON.stringify(data));
  var url = 'http://192.168.1.3:5000/form/save-form-degradation';
  $.ajax({
    url: url,
    type: 'POST',
    dataType: 'json',
...
function getData(token, data){
  var url = 'http://192.168.1.3:5000/form/get-form-degradation';
   $.ajax({
     url: url,
     type: 'POST',
     dataType: 'json',
...
function saveData(){
  ...
  const data = {
    'missionId': getUrlParameter('missionId'),
    'photoId': getUrlParameter('photoId'),
